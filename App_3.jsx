...
function MarcoRow({ marco, marcoState, onUpdate, eixoCor, isNew, isAdmin, onEdit, onDelete }) {
  ...
  const prazoAjustado = marcoState.prazo || "";
...
function parsePrazo(p) {
  if (!p) return null;
  // Marcos entregues antes da publicacao do plano (mai/2025) vem como "OK"
  if (p === "OK") return { mes: 4, ano: 2025 };
...
// Anos cobertos pela aba Monitoramento; abre no ano vigente (limitado a esse intervalo)
const ANOS_MONITORAMENTO = [2025, 2026, 2027, 2028];
function anoInicialMonitoramento() {
  const atual = new Date().getFullYear();
  if (ANOS_MONITORAMENTO.includes(atual)) return atual;
  return atual < ANOS_MONITORAMENTO[0]
    ? ANOS_MONITORAMENTO[0]
    : ANOS_MONITORAMENTO[ANOS_MONITORAMENTO.length - 1];
}
...
function MonitoramentoTab({ eixosData, marcoState, indState }) {
  const [selectedAno, setSelectedAno] = useState(anoInicialMonitoramento());
...
export default function App() {
      ...
      .map(e => ({
        ...
        diretrizes: e.diretrizes.map(d => ({
          ...
          marcos: d.marcos.filter(m => {
            ...
            const msPrazo = (marcoState[m.id] || {}).prazo;
            const prazoEfetivo = msPrazo ? msPrazo : m.prazoOriginal;
            const parsed = parsePrazo(prazoEfetivo);
  ...
  return (
    <div style={{ fontFamily: "'Inter', 'Segoe UI', system-ui, sans-serif", minHeight: "100vh", background: "#F0F2F5" }}>
      ...
      {/* ── Cabeçalho fixo (header + faixa + abas) ── */}
